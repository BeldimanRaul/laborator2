...
//      METODA DE SCRIERE SIMPLA , NU FOLOSESTE THREADURI (LABORATOR 7)
...
//        csv.ScrieCurs(cursnou);
        ...
        Runnable citireTask = () -> {
            System.out.println("Începe citirea ");
            csv.CitesteDate();
            System.out.println("Citirea datelor e gata");
        };


        Runnable scriereTask = () -> {
            System.out.println("Încep scrierea datelor");
            csv.ScrieStud(new Student("Colegu", "Nou", 12));
            csv.ScrieStud(new Student("Colegu", "Mai Nou", 12));
            csv.ScrieProf(new Profesor("Profesor", "Nou"));
            csv.ScrieCurs(new Curs("Curs Nou", "Descriere"));
            System.out.println("Scrierea datelor s-a încheiat.");
        };


        ExecutorService executor = Executors.newFixedThreadPool(2);
        Thread citireThread = new Thread(citireTask);
        Thread scriereThread = new Thread(scriereTask);
        citireThread.start();
        scriereThread.start();
